...
#%% Solve for sigma
impath = "./data/PODG/FOTR/adaptive/refine=nth/Nm=150/"
immpath = "./plots/PODG_1D/FOTR/adaptive/refine=nth/Nm=150/"
...
for opt_step in range(max_opt_steps):
    ...
    if stagnate > stag_ctr:
        ...
        if abs(dJ * 100) < 1e-5:
            print("\n\n-------------------------------")
            print("WARNING: dJ between subsequent basis refinement steps has turned close to 0...")
            print(
...
# import numpy as np
# lamda = {'q_reg': 1e-3}
# impath = "./data/PODG/normal/refine=nth/Nm=100/"
# J = np.load(impath + 'J_opt_list.npy')
# itr = np.load(impath + 'basis_refine_itr_list.npy')
# print(J.shape)
# print(itr.shape)
# qs_target = np.load(impath + 'qs_target.npy')
# f = np.load(impath + 'f_opt.npy')
# q0 = wf.InitialConditions_primal()
# qs_opt_full = wf.TimeIntegration_primal(q0, f, A_p, np.ones_like(f), ti_method=tm)
#
# J_opt = Calc_Cost(qs_opt_full, qs_target, f, lamda, **kwargs)
#
# print(J_opt)
