...
class Wildfire:
    ...
    def RHS_primal(self, q, f, sigma):
        ...
        Tdot = Coeff_diff * self.Mat_primal.Laplace.dot(T) - DT.dot(T) - Coeff_source * T + Coeff_arrhenius * \
               arrhenius_activate * S * jnp.exp(-self.__mu / (jnp.maximum(T, epsilon))) + m_T * f_T
        Sdot = - Coeff_massfrac * arrhenius_activate * S * jnp.exp(-self.__mu / (jnp.maximum(T, epsilon))) + m_S * f_S
    ...
    def RHS_adjoint(self, q_adj, f, TS, TS_tar, T_var, S_var):
        ...
        T_adj_dot = - self.__k * self.Mat_adjoint.Laplace.dot(T_adj) - DT.dot(T_adj) - arrhenius_activate * \
                    ((self.__alpha * self.__mu * S * jnp.exp(-self.__mu / (jnp.maximum(T, epsilon))) * T_adj / (jnp.maximum(T, epsilon)) ** 2)
                     + (self.__alpha * jnp.exp(-self.__mu / (jnp.maximum(T, epsilon))) * S_adj)) + self.__alpha * self.__gamma * T_adj - \
                    T_var * (T - T_tar)
        S_adj_dot = arrhenius_activate * (
                (self.__gamma_s * self.__mu * S * jnp.exp(-self.__mu / (jnp.maximum(T, epsilon))) * T_adj / (jnp.maximum(T, epsilon)) ** 2) +
                (self.__gamma_s * jnp.exp(-self.__mu / (jnp.maximum(T, epsilon))) * S_adj)) - S_var * (S - S_tar)
